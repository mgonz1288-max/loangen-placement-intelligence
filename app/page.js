...
const lenders = [
  { rank: 1, name: 'Apex Medical Finance', type: 'Healthcare equipment specialist', probability: 94, time: '2–4 days', reason: 'Exact industry and asset match. Accepts the requested amount with strong cash-flow coverage and a 742 guarantor credit score.', criteria: ['Healthcare', 'MRI equipment', '1.42x coverage', '6+ years'], action: 'Best submission' },
  { rank: 2, name: 'Northview Capital', type: 'General equipment lender', probability: 89, time: '3–5 days', reason: 'Fits the revenue, time-in-business, credit, and equipment-life requirements. Slightly higher expected pricing than Apex.', criteria: ['$2.84M revenue', '742 credit', '60–72 months'], action: 'Strong backup' },
  { rank: 3, name: 'Summit Commercial Finance', type: 'Middle-market equipment lender', probability: 86, time: '3–6 days', reason: 'Strong transaction-size and debt-service fit. Healthcare is accepted, but not a dedicated specialty program.', criteria: ['$385K request', 'Positive cash flow', 'Essential equipment'], action: 'Third option' },
  { rank: 4, name: 'Velocity Equipment Funding', type: 'Fast-close equipment program', probability: 82, time: '1–3 days', reason: 'Fast credit process and acceptable guarantor profile. Shorter preferred term may increase the monthly payment.', criteria: ['Fast close', '700+ credit', 'Equipment only'], action: 'Speed option' },
  { rank: 5, name: 'Meridian Healthcare Capital', type: 'Healthcare growth lender', probability: 78, time: '5–8 days', reason: 'Strong healthcare and expansion-purpose match. Requires the final equipment invoice and referral-volume support.', criteria: ['Healthcare', 'Expansion', 'Invoice required'], action: 'Specialty option' },
  { rank: 6, name: 'Ironwood Business Credit', type: 'Cash-flow term lender', probability: 74, time: '4–7 days', reason: 'Cash flow supports the request, but the lender may require a blanket lien and additional tax-return documentation.', criteria: ['1.42x coverage', 'Tax returns', 'Blanket lien'], action: 'Broader structure' },
  { rank: 7, name: 'Harbor Federal', type: 'SBA preferred lender', probability: 71, time: '21–35 days', reason: 'Lower-payment alternative with longer amortization. Ranked lower because speed and documentation do not match the stated priority.', criteria: ['SBA eligible', 'Long amortization', 'Full documentation'], action: 'Payment option' },
  { rank: 8, name: 'Pioneer National Leasing', type: 'Equipment leasing platform', probability: 67, time: '5–9 days', reason: 'Asset is eligible, but the request is near the top of the preferred exposure range and may require additional collateral support.', criteria: ['Eligible asset', 'Exposure limit', 'Collateral review'], action: 'Conditional' },
  { rank: 9, name: 'Crestline Working Capital', type: 'Revenue-based lender', probability: 59, time: '1–2 days', reason: 'Revenue qualifies, but the product is a weaker fit for a long-life asset and would likely carry a higher payment and cost.', criteria: ['Revenue qualifies', 'Higher cost', 'Short term'], action: 'Fallback only' },
  { rank: 10, name: 'Atlantic Community Bank', type: 'Conventional bank term loan', probability: 48, time: '30–45 days', reason: 'Potentially competitive pricing, but existing equipment debt, collateral review, and conventional documentation reduce certainty and speed.', criteria: ['Bank pricing', 'Collateral review', 'Slow close'], action: 'Last option' },
];
    ...
    <main>
      ...
      {screen === 'report' &&
        <section className="report">
          ...
          <div className="section-heading"><div><Building2 size={19}/><span><small>LENDER PLACEMENT</small><h3>Underwriting-based lender placement</h3></span></div><p>Ten lenders ranked from strongest to weakest underwriting fit</p></div>
          <div className="lender-table card">
            <div className="table-head"><span>RANK / LENDER</span><span>UNDERWRITING FIT</span><span>EST. APPROVAL</span><span>EXPECTED TIME</span><span>PLACEMENT</span></div>
            {lenders.map((lender) => <div className={`lender-row ${lender.rank === 1 ? 'top-placement' : ''}`} key={lender.name}><div className="lender-name"><b>{lender.rank}</b><span>{lender.name.slice(0,2).toUpperCase()}</span><div><strong>{lender.name}</strong><small>{lender.type}</small></div></div><div className="underwriting-fit"><p>{lender.reason}</p><div>{lender.criteria.map(item => <span key={item}>{item}</span>)}</div></div><strong className="probability">{lender.probability}%</strong><span className="fund-time"><Clock3 size={13}/>{lender.time}</span><b className={lender.rank === 1 ? 'plan first' : 'plan'}>{lender.action}</b></div>)}
          </div>
